"""DELIBERATE SINGLE-CHANGE TEST: train_final.py's exact 622.14 config
(fixed manual hyperparameters, uniform DECAY=0.005, simple-average blend,
global isotonic calibration) with ONE change -- add the base matchup
feature only (round-3's add_matchup_columns_training, with the prior_mean
look-ahead leak already fixed -- see verify_extension_leakage.py). No
matchup extensions, no re-tuning, no segmented calibration, no stacker.
This isolates the matchup feature's real-world effect for the first time;
it was previously only ever submitted bundled with round-4's other changes.
...
# ONLY CHANGE vs train_final.py: add the base matchup feature (leak-fixed).
