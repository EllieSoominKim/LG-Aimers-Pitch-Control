"""PHASE 1 DIAGNOSTIC-VALIDATED CANDIDATE: train_final.py's exact 622.14
config (fixed manual hyperparameters, uniform DECAY=0.005, simple-average
blend, global isotonic calibration) with matchup + matchup extensions
added -- the two changes confirmed STABLE across all 3 rolling-origin
folds in rolling_origin_validation.py (matchup: +0.00104 mean, std=0.00050,
same sign in all 3 folds; extensions: +0.00101 mean, std=0.00009, the most
stable change measured). Per-model Optuna re-tuning is DELIBERATELY
EXCLUDED -- rolling_origin_validation.py found it unstable (sign flip in
fold1, near-zero mean effect +0.00001) and asof_n_ablation.py's manual-
hyperparameter leg scored slightly BETTER on 3-fold mean than the tuned
version (0.00975 vs 0.00964). Segmented calibration and the logistic
stacker are also excluded -- neither was validated across the 3 rolling
folds, so per the user's explicit instruction this submission sticks to
exactly what has been validated stable.
...
# Confirmed-stable changes vs train_final.py: base matchup + matchup
# extensions (leak-fixed). Column ORDER matters for CatBoost -- these are
# appended AFTER build_features() returns, so they end up last, matching
# script_template_matchup_ext.py's ALL_FEATURE_COLS exactly.
