"""Generate self-contained base64 blobs for the matchup-only single-change
test: categories + calibration -> one aux blob, the matchup lookup table
-> its own blob, plus one independent blob per model file. No matchup
extension lookup, no stacker -- those don't exist in this version.
"""
...
out_path = os.path.join(OUT_DIR, "_embedded_artifacts_snippet_matchup_only.py")
