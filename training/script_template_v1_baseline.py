...
#
# DELIBERATE REVERSION (see training/train_final.py, unmodified since it
# was first written): this is the pre-round-3 model that scored 622.14 on
# the leaderboard -- no pitcher-batter matchup feature (added in round 3,
# never itself submitted in isolation), no per-model Optuna tuning (added
# in round 3/4), no cold/warm segmented calibration or logistic stacker
# (added in round 4). Simple average of 3 globally-calibrated
# gradient-boosted trees (LightGBM, CatBoost, sklearn
# HistGradientBoostingClassifier), trained on train.csv seasons 2019-2023
# with a single fixed recency-decay (0.005) applied uniformly to all three
# models, calibrated on season 2024.
#
# The ONLY things different from the exact 622.14 submission are the
# defensive/packaging fixes developed in round 2 (BASE_DIR path anchoring,
# case-insensitive artifact resolution, every model/ artifact embedded
# directly in this file as a gzip+base64 fallback) -- see
# training/train_final.py for the unmodified training script and
# training/generate_embedded_artifacts.py / training/build_submit_zip.py
# for how this file and submit.zip were rebuilt.
#
# Every feature is a deterministic, row-local transform of columns
# documented as available before the pitch is thrown (see
# data_description.md). No cross-row aggregation is performed on
# data/test.csv.
...
# isotonic calibration breakpoints) and one independent blob per model file
# (lightgbm.txt, hgb.joblib, catboost.cbm). Every one of them is tried from
# model/ first (case-insensitive-tolerant) and falls back to its embedded
# copy on any failure -- see the top-of-file comment for why.
# ---------------------------------------------------------------------
...
def build_features(df, categories):
...
def main():
    ...
    X = build_features(test, categories)
