...
# FIXED climatology reference: unweighted TR average, constant across all
# decay/config comparisons (this is what makes BSS numbers comparable).
P_CLIM = y_tr.mean()
print(f"Fixed climatology reference (unweighted TR mean) = {P_CLIM:.4f}")

results = []
for decay in [0.2, 0.1, 0.05, 0.03, 0.02, 0.01, 0.005, 0.001]:
    ...
    print(f"decay={decay:<7.3f} best_iter={m.best_iteration_:5d}  VAL Brier={bs:.5f}  BSS={bss:.5f}  ({time.time()-t0:.1f}s)")
