"""CatBoost re-tune, size-capped: item #1+#3's 400-trial search (depth up
to 10) found depth=10 optimal for CAL-objective BSS, but that produced a
304MB model file (vs 29MB at round-3's depth=7) -- a 10x blowup in
script.py size given the embedded-artifact-fallback design, an
unacceptable risk given how many submission-staging failures this project
has already hit. This search caps depth<=8 and uses the matchup+extensions
feature set (now the validated best config, unlike tune_catboost_v2.py
which only had base matchup), otherwise identical methodology (pruned
Optuna, persistent SQLite storage, per-trial logging).
"""
import json
import sys
import time
...
CHECKPOINT = int(sys.argv[2]) if len(sys.argv) > 2 else 60
MAX_DEPTH = 8
...
study = optuna.create_study(
    ...
    storage=storage, study_name="catboost_v3", load_if_exists=True,
)
...
# Report the trained model's on-disk size so this stays visible going
# forward, not just discovered again the next time this is retrained.
import tempfile, os
with tempfile.TemporaryDirectory() as tmpdir:
    tmp_path = os.path.join(tmpdir, "catboost_probe.cbm")
    cb.save_model(tmp_path)
    size_mb = os.path.getsize(tmp_path) / 1e6
print(f"Trained model size: {size_mb:.1f} MB  (depth cap was {MAX_DEPTH})")
...
with open("C:/LG-Aimers-Pitch-Control/training/best_catboost_params_v3.json", "w") as f:
    json.dump({"decay": decay, **best, "val_bss_cal_honest": bss_val_cal,
               ...
               "checkpoint_trials": CHECKPOINT, "total_trials": N_TRIALS,
               "model_size_mb": size_mb}, f, indent=2)
