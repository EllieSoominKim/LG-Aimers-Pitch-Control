...
#
# PHASE 1 DIAGNOSTIC-VALIDATED CANDIDATE (see training/train_final_matchup_ext.py):
...
# matchup + matchup extensions added -- both confirmed STABLE across all 3
# rolling-origin folds (training/rolling_origin_validation.py: matchup
# +0.00104 mean/std=0.00050 same sign every fold; extensions +0.00101
# mean/std=0.00009, the most stable change measured in that pass).
# Per-model Optuna re-tuning is DELIBERATELY EXCLUDED -- it was the one
# change with an unstable sign (negative in one rolling fold, near-zero
# mean effect), and asof_n_ablation.py's manual-hyperparameter leg scored
# slightly BETTER on 3-fold mean than the tuned version anyway. Segmented
# calibration and the logistic stacker are also excluded -- neither was
# validated across the 3 rolling folds, so this submission sticks to
# exactly what's been validated stable.
#
...
# isotonic calibration breakpoints), the matchup + matchup-extension lookup
# tables, and one independent blob per model file (lightgbm.txt,
...
def _load_embedded_matchup_ext():
    """Decode the embedded matchup extension lookup table."""
...
# Pitcher-batter matchup history + extensions (last-3-meetings rolling
# trend, pitcher-vs-batter-hand shrunk rate, matchup-rate x own-asof-rate
# interactions). Static lookup tables built entirely from train.csv
# (2019-2024) -- test.csv rows never contribute to them. Column ORDER
# matters for CatBoost (fixed positional schema at training time); the
# training pipeline (train_final_matchup_ext.py) appends these columns
# AFTER categories.json's categorical columns, so they must come LAST here
# too, in this exact order: 38 raw numeric, 4 engineered numeric, 11
...
def add_matchup_extension_columns_inference(df, ext_table):
    """Static lookups for the matchup extension columns, built entirely
    from train.csv (2019-2024). Requires MATCHUP_SHRUNK_COL already
    present (add_matchup_columns_inference must run first) -- the
    interaction terms are computed directly from already-loaded features,
    no lookup needed for those two."""
    df = df.copy()
